...
const roleCosts = {
  ...
  // Nombrado - Light (0.5 points)
  'Oracion inicial': 0.5,
  'Oracion final': 0.5,
  'Acomodadores exterior': 0.5,
  'Acomodadores interior': 0.5,
  // Matriculado (2 points)
  'Sala A Lectura Biblia': 2,
  'Sala B Lectura Biblia': 2,
  'Sala A Asignacion': 2, // Generic cost for pairs
  'Sala B Asignacion': 2, // Generic cost for pairs
};
...
const AssignmentScheduler = () => {
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          if (parsed.length > 0 && parsed[0].data) {
            console.log("Old history format detected. Clearing history to prevent errors. Please re-approve past months if needed.");
            setHistory([]);
          } else {
            setHistory(parsed);
          }
  ...
  const roles = [
    'Presidente', 'Oracion inicial', 'Tesoros', 'Perlas', 'Vida y ministerio', 'Vida y ministerio 2',
    'Estudio biblico', 'Lector del libro', 'Oracion final', 'Acomodadores exterior', 'Acomodadores interior'
  ];
  ...
  const addPerson = () => {
    if (newPersonName.trim()) {
      const newPerson = { id: Date.now(), name: newPersonName.trim(), roles: {} };
      roles.forEach(role => { newPerson.roles[role] = false; });
  ...
  const toggleAllRoles = (personId, assign) => {
    setPeople(people.map(person => 
      person.id === personId 
        ? { ...person, roles: Object.keys(person.roles).reduce((acc, role) => ({ ...acc, [role]: assign }), {}) }
        : person
    ));
  };
  ...
  const generateAssignments = () => {
      ...
      if (lastWeekKey) {
        ...
        Object.values(lastWeekAssignments).forEach(assignment => {
          ...
          if (typeof assignment === 'string') {
            usedInLastWeekOfPrevMonth.add(assignment);
          } else if (Array.isArray(assignment)) {
            assignment.forEach(name => usedInLastWeekOfPrevMonth.add(name));
          } else if (typeof assignment === 'object' && 'encargado' in assignment) {
            usedInLastWeekOfPrevMonth.add(assignment.encargado);
            usedInLastWeekOfPrevMonth.add(assignment.ayudante);
          }
        });
    ...
    matriculados.forEach(person => {
      ...
      if (hadAsignacionLastMonth && hadAsignacionTwoMonthsAgo) {
        mustRestMatriculados.add(person.id);
      }
    ...
    const getNextPerson = (candidates, { role, weekIndex, usedInWeek, isPairAssignment = false, pairRoleType = null }) => {
      let roleCost;
      let isMatriculadoRole = false;
      if (isPairAssignment) {
        roleCost = roleCosts['Sala A Asignacion'];
        isMatriculadoRole = true;
      } else {
        roleCost = roleCosts[role] || 1;
        if (role.includes('Lectura Biblia')) isMatriculadoRole = true;
      }
      ...
      const scored = eligible.map(p => {
        ...
        if (isPairAssignment && pairRoleType === 'encargado' && matriculadoRotationInfo[p.id] === 'ayudante') {
          score += 50;
        }
    ...
    weeks.forEach((week, weekIndex) => {
      ...
      const assignGroup = (role, count) => {
        ...
        for (let i = 0; i < count; i++) {
          const person = getNextPerson(candidates, { role, weekIndex, usedInWeek });
          if (person) {
            assignedPeople.push(person.name);
    ...
    Object.values(assignments).forEach(weekAssignments => {
      Object.entries(weekAssignments).forEach(([role, assignment]) => {
        if (typeof assignment === 'object' && assignment !== null && 'encargado' in assignment) {
          ...
          if (encargadoPerson) {
            if (!newMonthMatriculadoHistory[encargadoPerson.id]) newMonthMatriculadoHistory[encargadoPerson.id] = { encargado: 0, ayudante: 0 };
            newMonthMatriculadoHistory[encargadoPerson.id].encargado += 1;
          }
          if (ayudantePerson) {
            if (!newMonthMatriculadoHistory[ayudantePerson.id]) newMonthMatriculadoHistory[ayudantePerson.id] = { encargado: 0, ayudante: 0 };
  ...
  const exportData = () => {
    const dataToExport = { people, matriculados, history, matriculadoHistory };
  ...
  const getSortedRolesForDisplay = (weekAssignments) => {
    ...
    const finalOrder = [
      'Presidente', 'Oracion inicial', 'Tesoros', 'Perlas', ...lecturaBiblia, ...salaA, ...salaB,
      'Vida y ministerio', 'Vida y ministerio 2', 'Estudio biblico', 'Lector del libro', 'Oracion final',
      'Acomodadores exterior', 'Acomodadores interior'
    ];
    ...
    <div className="max-w-6xl mx-auto p-6 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        <div className="flex gap-4 mb-6">
          ...
          <button onClick={clearHistory} className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 flex items-center gap-2">
            <RotateCcw className="w-4 h-4" /> Limpiar Historial
          </button>
          <button onClick={exportData} className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 flex items-center gap-2">
            Exportar
          </button>
          <button onClick={triggerImport} className="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 flex items-center gap-2">
            Importar
          </button>
        </div>

        <div className="flex border-b mb-6">
          <button onClick={() => setActiveTab('programa')} className={`px-4 py-2 text-lg font-semibold ${activeTab === 'programa' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500'}`}>
            Programa
          </button>
          <button onClick={() => setActiveTab('nombrados')} className={`px-4 py-2 text-lg font-semibold ${activeTab === 'nombrados' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500'}`}>
            Nombrados
          </button>
          <button onClick={() => setActiveTab('matriculados')} className={`px-4 py-2 text-lg font-semibold ${activeTab === 'matriculados' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500'}`}>
            Matriculados
          </button>
          <button onClick={() => setActiveTab('historial')} className={`px-4 py-2 text-lg font-semibold ${activeTab === 'historial' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500'}`}>
            Historial
          </button>
        </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {roles.map(role => (
                      <label key={role} className="flex items-center gap-2 cursor-pointer">
                        <button onClick={() => toggleRole(person.id, role)} className="flex items-center">
                          {person.roles[role] ? <CheckSquare className="w-4 h-4 text-blue-600" /> : <Square className="w-4 h-4 text-gray-400" />}
                        </button>
                ...
                <div key={person.id} className="bg-white p-3 rounded-lg border">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                      <select value={person.gender} onChange={(e) => updateMatriculadoGender(person.id, e.target.value)} className={`border rounded-md py-1 px-2 text-sm ${person.gender === 'hombre' ? 'bg-blue-100 text-blue-800' : 'bg-pink-100 text-pink-800'}`}>
                        <option value="hombre">Hombre</option>
                        <option value="mujer">Mujer</option>
                      </select>
                  ...
                  {person.gender === 'hombre' && (
                    <div className="mt-2 pt-2 border-t border-gray-200">
                      <label className="flex items-center gap-2 cursor-pointer text-sm">
                        <button onClick={() => toggleMatriculadoRole(person.id, 'lecturaBiblia')} className="flex items-center">
                          {person.roles?.lecturaBiblia ? <CheckSquare className="w-4 h-4 text-blue-600" /> : <Square className="w-4 h-4 text-gray-400" />}
                        </button>
                ...
                <div key={monthEntry.month} className="bg-white p-4 rounded-lg border">
                  ...
                  <div className="space-y-2">
                    ...
                    {Object.entries(monthEntry.data).map(([role, names]) => (
                      <div key={role} className="text-sm"><span className="font-medium">{role}:</span> {names.join(', ')}</div>
                    ))}
                  </div>
                </div>
              ))}
        ...
        <div className="flex gap-4 mb-6">
          <button onClick={generateAssignments} className="w-1/2 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold">
            Generar Asignaciones para {months[currentMonth - 1]} {currentYear}
          </button>
          <button onClick={approveAndSaveHistory} disabled={Object.keys(assignments).length === 0} className="w-1/2 px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-semibold disabled:bg-gray-400 disabled:cursor-not-allowed">
                    ...
                    return (
                      <div key={role} className="flex justify-between items-center py-2 border-b">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => copyToClipboard(copyText)} className="text-gray-500 hover:text-gray-700"><Copy className="w-4 h-4" /></button>
                        </div>
                      </div>
                    );
                  })}
